...
namespace Image
{
	...
	std::vector<char> CopyStream(IStream & stream);
	...
	Data FreeImageCodec::Decode(IStream & input) const
	{
		...
		auto buffer = CopyStream(input);
		FIMEMORY * memory = FreeImage_OpenMemory(reinterpret_cast<BYTE *>(buffer.data()), static_cast<DWORD>(buffer.size()));
	...
	std::vector<char> CopyStream(IStream & stream)
	{
		...
		while (!stream.AtEnd())
		{
			...
			if (bytesRead)
				resBuffer.insert(resBuffer.end(), rawBuffer, rawBuffer + bytesRead);
		}
